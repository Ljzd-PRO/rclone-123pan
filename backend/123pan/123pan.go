...
func init() {
	fs.Register(&fs.RegInfo{
		...
		Options: []fs.Option{
			...
			{Name: "pass", Help: "123 网盘账号密码。", Required: true, IsPassword: true, Sensitive: true},
			{Name: "access_token", Help: "缓存的个人账号访问 token。", Sensitive: true, Hide: fs.OptionHideBoth},
			{Name: "root_folder_id", Help: "作为账号根目录使用的数字文件夹 ID。", Default: "0", Advanced: true, Sensitive: true},
			{Name: "platform", Help: "平台请求头；协议签名始终固定使用 web/3。", Default: "web", Advanced: true},
			{Name: "upload_concurrency", Help: "单个文件同时上传的数据分片上限。", Default: 3, Advanced: true},
			{Name: "hash_memory_limit", Help: "必须计算 MD5 时，允许在内存中缓存的最大源文件大小。", Default: fs.SizeSuffix(10 * fs.Mebi), Advanced: true},
			{Name: "api_min_interval", Help: "同一控制端点两次请求之间的最小间隔。", Default: fs.Duration(700 * time.Millisecond), Advanced: true},
			{Name: "verify_timeout", Help: "等待写操作终态成立的最长时间。", Default: fs.Duration(60 * time.Second), Advanced: true},
			{Name: "encoding", Help: "后端使用的文件名编码规则。", Default: defaultEncoding, Advanced: true},
		},
	})
}
