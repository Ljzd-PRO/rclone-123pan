...
2026-08-14 已使用一个非专用账号完成受限的隔离冒烟测试。测试只创建随机命名目录和小型临时文件，未使用任何既有文件；两个测试目录均在确认可见条目为零后删除，并按名称和 ID 从账号根确认不再可见。上传协议未通过，所以没有继续执行下载、更新、移动、离线任务、mount 或完整 rclone 契约测试。详细结果见[真实账号测试记录](live-testing.md)。
