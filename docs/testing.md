...
`tools/test-install.sh` 使用隔离的状态化下载替身验证一键安装器，包括最新正式 Release 解析、平台与架构映射、SHA-256 校验、候选版本核验、原程序一次性备份、原子替换、幂等重跑、指定标签、不支持平台失败关闭，以及任何请求均不访问源码分支。CI 与 Release 质量门禁都会执行该测试。
