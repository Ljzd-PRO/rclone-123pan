...
2026-08-15 已在同一授权账号的全新随机 anchor 中，通过官方 Web 上传新建的 1 KiB 和 16 MiB+1 文件并取得脱敏的单片/多片/秒传线型。修复后的后端已经完成 1 KiB 普通上传、秒传、0/1/16 MiB−1/16 MiB/16 MiB+1 边界、完整下载 MD5、可恢复 Update、同 ID Move/DirMove、非空 Rmdir 拒绝、精确软删除、双哨兵复核，以及同尺寸不同内容的 `check --checksum` 只读回归。48 MiB+1 并发、160 MiB+1 第二批 URL、101 项分页、离线任务、mount 和完整 rclone 契约测试仍须按门禁继续。详细记录见[真实账号测试记录](live-testing.md)。
