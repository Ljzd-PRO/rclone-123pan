...
包含账号安全风险、陌生设备、短信验证码或微信登录要求的服务端响应会转换为 fatal 的认证挑战错误，并在 HTTP pacer 和密码重登之前停止。运行时文件操作不提供交互式验证码入口，避免无人值守的 sync、mount、serve 或 RC 任务阻塞或反复触发风控。用户只能在 123 网盘官方网站完成验证，再显式运行 `rclone config reconnect <remote>:`。reconnect 先在内存 staging 中验证新 token 对应的 UID，成功后才原子更新隐藏配置；失败不会清空或覆盖原 token。
...
一键安装脚本会以安装目标所需的权限运行，因此其网络目标固定为本项目公开 GitHub Releases，不接受 endpoint 或仓库覆盖，也不依赖受匿名限额约束的 GitHub REST API。脚本只接受严格的正式版本标签格式，只从归档中提取精确的 `rclone` 路径，并在覆盖前同时验证 Release 中的 SHA-256 和候选程序报告的版本。摘要可以发现传输损坏或资产错配，但不能替代对 GitHub 发布账号本身的信任；要求更强审计时应先独立下载并检查 `install.sh`、`SHA256SUMS`、SBOM 和 provenance，再执行安装。
