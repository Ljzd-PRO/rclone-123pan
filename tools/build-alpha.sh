...
provenance="$dist/rclone_${product_version}.provenance.json"
...
echo "$targets" | while read -r goos goarch; do
  package="rclone_${product_version}_${goos}_${goarch}"
  package_dir="$stage/$package"
  mkdir -p "$package_dir"
  binary="$package_dir/rclone"
  binary_name=rclone
  ...
  if [ "$goos" = linux ]; then
    deb="$dist/rclone_${product_version}_${goos}_${goarch}.deb"
    ./tools/build-deb.sh \
