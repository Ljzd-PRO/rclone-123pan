...
- 增加短信验证码、微信登录、陌生设备和账号风控响应的 typed/fatal 识别，在 pacer 与 401 密码重登前失败关闭；增加 `rclone config reconnect` 的事务式重新认证，只有新 Token 通过用户信息验证后才覆盖隐藏配置，运行时传输保持完全非交互。
...
- 修复 GitHub 固定版本静态检查发现的资源关闭与临时文件清理结果处理，并将旧式 S3 SDK 弃用例外限制在协议兼容路径内。
- 修复 Release 元数据步骤的 ShellCheck 输出分组告警，使带 ShellCheck 的 actionlint 门禁保持全绿。
