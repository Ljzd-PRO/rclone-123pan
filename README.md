...
> [!WARNING]
> 本项目刚完成初步开发，不保证所有功能均能正常使用。目前的验证主要来自自动化测试，以及由 Agent 在授权真实账号的隔离临时目录中执行的有限测试，尚未经过广泛用户测试或长期生产环境验证。
>
...
123 网盘个人盘 API 的协议结构与行为参考了 [OpenList](https://github.com/OpenListTeam/OpenList) 的 123 网盘 driver；本项目结合官方 Web 客户端的实际请求进行了独立实现和验证。
