...
> [!WARNING]
> 当前版本是内部 alpha，使用逆向分析所得的 Web API，尚未通过专用真实测试账号验证，不适合公开发布或生产使用。
...
| AWS SDK v2 旧式上传和有界预签名分片上传 | 已实现并通过单元测试 |
...
预期行为和发布门禁详见[后端说明](docs/123pan.md)、[安全说明](docs/security.md)和[测试说明](docs/testing.md)。替换失败后的处理见[恢复与回滚](docs/recovery.md)。
